...
export async function getCurrentUserClinicRole() {
  ...
  return (rows as { role: AdminRole; is_owner: boolean; clinic_id: string }[])[0] ?? null
}

export async function requireClinicAccess(allowedRoles?: AdminRole[]): Promise<{ admin: NonNullable<Awaited<ReturnType<typeof getCurrentAdminProfile>>>; clinic: ClinicRow }> {
  const admin = await getCurrentAdminProfile()
  if (!admin) redirect('/admin')
  const clinic = await requireCurrentClinic()
  if (allowedRoles?.length && !allowedRoles.includes(admin.role)) redirect('/admin/dashboard')
  return { admin, clinic }
}
